...
-- Compatible with MySQL 5.7+ (IF NOT EXISTS on ADD COLUMN requires MySQL 8.0.3+)
DROP PROCEDURE IF EXISTS add_profile_columns;
DELIMITER $$
CREATE PROCEDURE add_profile_columns()
BEGIN
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'gender') THEN
        ALTER TABLE users ADD COLUMN gender ENUM('Nam', 'Nữ', 'Khác') DEFAULT NULL AFTER avatar;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'age') THEN
        ALTER TABLE users ADD COLUMN age TINYINT UNSIGNED DEFAULT NULL AFTER gender;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'description') THEN
        ALTER TABLE users ADD COLUMN description TEXT DEFAULT NULL AFTER age;
    END IF;
END$$
DELIMITER ;
