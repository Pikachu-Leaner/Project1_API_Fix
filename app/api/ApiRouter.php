...
class ApiRouter {
    ...
    public function dispatch(array $segments): void {
        ...
        try {
            match ($resource) {
                ...
                'products' => $this->routeProducts($method, $segments, $id),
                'categories' => $this->routeCategories($method, $segments, $id),
                'users' => $this->routeUsers($method, $segments, $id),
                'cart' => $this->routeCart($method, $segments),
                'orders' => $this->routeOrders($method, $segments, $id, false),
    ...
    private function routeAuth(string $method, array $s): void {
        ...
        match ([$method, $action]) {
            ...
            ['PATCH', 'reset-password'], ['POST', 'reset-password'] => $this->auth->resetPassword(),
            ['GET', 'me'] => $this->auth->me(),
            default => Response::error('Auth endpoint not found.', 404),
        };
    }

    private function routeProducts(string $method, array $s, ?int $id): void {
    ...
    private function routeCategories(string $method, array $s, ?int $id): void {
        if ($method === 'GET' && $id === null) $this->categories->index();
        Response::error('Use /api/admin/categories for category write methods.', 405);
    }

    private function routeUsers(string $method, array $s, ?int $id): void {
        if (($s[2] ?? '') === 'me') {
            match ($method) {
                'GET' => $this->users->me(),
    ...
    private function routeOrders(string $method, array $s, ?int $id, bool $adminRoute): void {
    ...
    private function routeAdmin(string $method, array $s): void {
        ...
        if ($area === 'categories') {
            ...
            if (($method === 'PUT' || $method === 'PATCH') && $id !== null) $this->categories->update($id);
            if ($method === 'DELETE' && $id !== null) $this->categories->delete($id);
        }

        if ($area === 'users') {
            if ($method === 'GET' && $id === null) $this->users->index();
            if ($method === 'GET' && $id !== null) $this->users->show($id);
            if ($method === 'PATCH' && $id !== null && ($s[4] ?? '') === 'status') $this->users->toggleStatus($id);
            if (($method === 'PATCH' || $method === 'PUT') && $id !== null) $this->users->update($id);
        }

        if ($area === 'orders') {
            $orderId = isset($s[3]) && ctype_digit((string)$s[3]) ? (int)$s[3] : null;
            $this->routeOrders($method, array_merge(['api', 'orders'], array_slice($s, 3)), $orderId, true);
        }
