    ...
    app.innerHTML = `
        <div class="auth-card bg-white p-4 rounded shadow-sm">
            ...
            <div class="vstack gap-3">
                ...
                <textarea class="form-control" id="pr-addr" placeholder="Địa chỉ">${esc(state.user.address||'')}</textarea>
                <input class="form-control" id="pr-avatar"  value="${esc(state.user.avatar||'')}"  placeholder="Avatar URL">
                <button class="btn btn-primary fw-bold" id="pr-save">Lưu bằng PATCH API</button>
            </div>
        </div>`;
    document.querySelector('#pr-save').addEventListener('click', async () => {
        const body = {
            full_name: document.querySelector('#pr-name').value,
            phone:     document.querySelector('#pr-phone').value,
            address:   document.querySelector('#pr-addr').value,
            avatar:    document.querySelector('#pr-avatar').value,
        };
