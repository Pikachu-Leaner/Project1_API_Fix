...
async function api(path, options = {}) {
    ...
    try {
        ...
        throw new Error('API không trả JSON. Có thể bạn đang mở sai URL hoặc Apache chưa rewrite vào index.php. URL API: ' + API_BASE + path);
    }
    if (!response.ok || payload.success === false) {
        let message = payload.message || `HTTP ${response.status}`;
        if (payload.details && payload.details.error) message += ' Chi tiết: ' + payload.details.error;
...
async function renderProducts() {
    ...
    app.innerHTML = '<div class="text-center py-5 text-muted"><div class="spinner-border"></div><p class="mt-3">Đang tải sản phẩm...</p></div>';
